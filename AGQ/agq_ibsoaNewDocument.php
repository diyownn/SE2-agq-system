...
function updateRecord($conn, $data, $sessionData)
{
    ...
    $stmt->bind_param(
        "sssssssssssssssisiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiissssssss",
        $data['to'],
        $data['address'],
        $data['tin'],
        $data['attention'],
        ...
        $data['95oceanfreight'],
        $data['forwarder'],
        $data['warehousecharges'],
        $data['e-lodgement'],
        $data['processing'],
        $data['customsformsstamps'],
        $data['photocopynotarial'],
        $data['documentation'],
        $data['deliveryexpense'],
        $data['misc.transpo.tel.card'],
        $data['doortodoorbacolod'],
        $data['arrastrewharfagestorage'],
        $data['thc-shippingline'],
        $data['aisl'],
        $data['gofast'],
        $data['additionalprocessing'],
        $data['extrahandlingfee'],
        $data['clearanceexpenses'],
        $data['haulingandtrucking'],
        $data['additionalcontainer'],
        $data['handling'],
        $data['stuffingplant'],
    ...
    <a href="#" onclick="return redirection('<?php echo htmlspecialchars($refNum, ENT_QUOTES, 'UTF-8'); ?>')" style="text-decoration: none; color: black; font-size: x-large; position: absolute; left: 20px; top: 20px;">←</a>
        <div class="container">
            ...
            <div class="section">
                ...
                <input type="text" maxlength="25" name="edited" placeholder="Edited by" value="<?= isset($row['Edited_by']) ? htmlspecialchars($row['Edited_by']) : ''; ?>" onchange="validateTextFields(this)" style="width: 48%">
            </div>
            <div class="footer">
